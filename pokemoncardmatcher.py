...
class PokemonCardMatcher:
    ...
    def validate_columns(self, required_columns) -> bool:
        missing_columns = [col for col in required_columns if col not in self.df.columns]
        if missing_columns:
            raise ValueError(f"The following required columns are missing from the CSV: {', '.join(missing_columns)}")

    def get_top_n_matches(self, image: Image, n=5, hash_size=16, algorithm=ImageHashAlgorithm.PHASH) -> List[Dict[str, float]]:
